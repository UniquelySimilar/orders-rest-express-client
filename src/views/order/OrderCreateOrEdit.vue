    ...
    <form>
      <div class="form-group row">
        ...
        <div class="col-md-4">
          <select id="orderStatus" v-model="order.order_status">
            <option v-for="status in orderStatusList" v-bind:value="status.value" v-bind:key="status.value">
              {{status.text}}</option>
          </select>
        </div>
      </div>
      ...
      <div class="form-group row" v-if="orderId">
        ...
        <div class="col-md-3">
          <datepicker id="shippedDate" :dateFormat="dateFormat" :dateType="3" :initialDate="order.shipped_date"
            v-on:update-date="updateDate"></datepicker>
          <button type="button" class="btn btn-outline-dark btn-xs btn-margin-left"
            @click="clearShippedDate">Clear</button>
        </div>
        <div class="col-md-4 error-msg">
          <span>*&nbsp;</span>
      ...
      <div class="form-group row">
        <div class="col-md-offset-4 col-md-2">
          ...
          <router-link class="btn btn-outline-dark" :to="{ name: 'customerDetailOrders', params: { id: customerId } }">
            Cancel</router-link>
        </div>
      </div>
    </form>
